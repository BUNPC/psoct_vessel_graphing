function [wm_mask, gm_mask, tissue_mask] = create_wm_mask(mus,oct,th)
...
figure; imshow(wm_mask); title('Border Eroded')

% Remove voxels with fewer than nmin connections
nmin = 10000;
wm_mask = bwareaopen(wm_mask, nmin);
figure; imshow(wm_mask); title('WM Mask - Islands removed')
...
figure; imagesc(tissue_mask); title('Threshold for agarose');
% Erode the borders
tissue_mask = imerode(tissue_mask, se_erode);
figure; imagesc(tissue_mask); title('Erode tissue borders');
% Close the holes
tissue_mask = imfill(tissue_mask, 'holes');
figure; imagesc(tissue_mask); title('Close holes');
...
figure; imshow(gm_mask); title('Tissue mask minus WM mask');

...
datapath  = 'E:\Jiarui\Data\code\fitting_code\';

% add subfunctions for the script
addpath('E:\Jiarui\Data\code\fitting_code');
...
%% this section is for parallelization on SCC only
% njobs=9;
% 
...
for iFile=1:length(filename0)
   ...
   % volumetric averaging before fitting
    ...
    % assume Rayleigh range is constant cross the field of view
    %load('Zf.mat');
    res = 10;
    ...
    for i = 1:round(size(I,2)/res)
        for j = 1:round(size(I,3)/res)
            ...
            if m > 10
                ...
                est_pix(i,j,:) = lsqcurvefit(fun_pix,[est(1) est(2) est(3)],z,ydata,lb,ub,opts);
            else
                est_pix(i,j,:) = [0 0 0];
            end
        end           

...
x_range = linspace(min(reducedData(:,1)), max(reducedData(:,1)), 500);
y_range = linspace(min(reducedData(:,2)), max(reducedData(:,2)), 500);
[X1, X2] = meshgrid(x_range, y_range);
grid_X = [X1(:), X2(:)];
[grid_clusters, ~] = kmeans(grid_X, k, 'MaxIter', 1, 'Start', C2);
% Plot the grid with cluster colors
figure;
gscatter(grid_X(:,1), grid_X(:,2), grid_clusters, [0,0.75,0.75;0.75,0,0.75;0.75,0.75,0], '..');
hold on;
% Plot PCA data points on top of the cluster-specific colored partitions
plot(reducedData(:,1), reducedData(:,2), 'k*', 'MarkerSize', 6, 'LineWidth', 3);
title('PCA followed by K-Means Clustering (2D)');
xlabel('Principal Component 1');
ylabel('Principal Component 2');
legend('Group 2','Group 1','Group 3','Data','Location','SouthEast');
set(gca, 'FontSize', 30)
hold off;
...
set(gca, 'FontSize', 20);   %increase font size to 30
set(findall(gca, 'Type', 'line'), 'LineWidth', 2); %increase line width

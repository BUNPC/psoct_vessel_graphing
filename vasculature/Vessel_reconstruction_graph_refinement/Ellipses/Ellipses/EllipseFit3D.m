function [s, valid] = EllipseFit3D(Vol, x, y, z)
% General ellipsoidfit in 3D 
...
% vessel is given by AS_RATIO*max( minor axes) and not estimated from the
% data.
...
for i = 1:Iteration
    ...
    s = UpdateScale(s,t);
    s = UpdatePosition(s,t);
    fprintf('Iter: %d  Rate = [%.02f, %.02f, %.02f, %.02f, %.02f, %.02f %.02f, %.02f, %.02f]\n', i,...
